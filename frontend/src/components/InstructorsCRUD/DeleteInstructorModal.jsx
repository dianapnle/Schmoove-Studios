...
function DeleteInstructorModal({ instructorId }) {
  ...
  if (sessionUser) {
    const deleteSubmit = (e) => {
      e.preventDefault()
      return dispatch(thunkDeleteInstructor(instructorId))
        .then(closeModal)
        .catch(async (res) => {
      ...
      <>
        <div className={`delete-modal`}>
          ...
          <div className={`buttons`}>
            <div>
              <button className={`delete-post-button`} onClick={deleteSubmit}>
                Yes (Delete Instructor){' '}
              </button>
            </div>
            <br></br>
            <div>
              <button className={`no-post-button`} onClick={closeModal}>
                No (Keep Instructor){' '}
              </button>
            </div>
          </div>
