...
import {
  thunkGetAllStudioInstructors,
  thunkCreateInstructor,
} from '../../store/instructors'
...
const AddInstructorModal = ({ studioId }) => {
  ...
  const foundUserIds = new Set(
    Object.values(filteredInstructors).map((instructor) => instructor.userId),
  )
  // filter out user objects that were already added (based on userId)
  const dropDownUsers = Object.values(allInstructors).filter(
    (user) => !foundUserIds.has(user.id),
  )

  useEffect(() => {
    const errors = {}

    if (userId === 'select_an_option')
      errors.user = 'An instructor must be selected!'
    if (profilePic === undefined)
      errors.profilePic = 'Image needed in .png, .jpg, or .jpeg'
    setErrors(errors)
  }, [userId, profilePic])
  ...
  if (sessionUser) {
    const handleSubmit = async (e) => {
      ...
      dispatch(thunkCreateInstructor(payload, profilePic, studioId)).then(() =>
        closeModal(),
      )
    }
      ...
      <div className='modal-instructors'>
        ...
        <form>
          <div className='add'>
            <div className='child'>
              <label>
                ...
                >
                  <option disabled selected value='select_an_option'>
                    {' '}
                    -- select an option --{' '}
                  </option>
          ...
          <div className='error-area'>
            {hasSubmitted === true && errors.user && (
              <div className={`errors`}>{errors.user}</div>
            )}
            {hasSubmitted === true && errors.profilePic && (
              <div className={`errors`}>{errors.profilePic}</div>
            )}
          </div>
        </form>
        <div className='child'>
