...
function ClassTile({ studioId, classId, showEdit }) {
  ...
  return (
    <div className={`classOverallContainer`}>
      ...
      {showEdit && sessionUser?.id === currentStudio?.ownerId && (
        <div className='top-container'>
          <OpenModalEditClassButton
            modalComponent={<EditClassModal classId={classId} />}
          />
          <OpenModalDeleteButton
            modalComponent={<DeleteClassModal classId={classId} />}
          />
        </div>
      )}
      <div
        onClick={() => {
          navigate(`/classes/${el.id}`)
        }}
        data-text={el?.name}
      >
        <div>
          <span>
            <h2>
              {el?.name} with {el?.Instructor?.firstName}
            </h2>
          </span>
          <h4>
            Styles: {el?.DanceStyles[0]?.name}, {el?.DanceStyles[1]?.name}
          </h4>
        </div>
